const Reports = {
  ...
  async load() {
    try {
      ...
      this.render(data);
      Dashboard.renderHero(data);
    } catch(err) { Utils.toast('Erro ao carregar relatório.','error'); }
  },
  render(data) {
    const onReports = !document.getElementById('page-reports')?.classList.contains('hidden');
    this.renderMonthly(data.monthly, onReports ? 'chart-monthly-r' : 'chart-monthly');
    if (onReports) { this.renderDonut(data.byCategory); this.renderPie(data); }
  },
  renderMonthly(monthly, canvasId) {
    ...
    const months=[];
    for(let i=5;i>=0;i--){ let mo=m-i,yr=y; if(mo<=0){mo+=12;yr--;} months.push({m:mo,y:yr,label:new Date(yr,mo-1).toLocaleDateString('pt-BR',{month:'short',year:'2-digit'})}); }
    ...
    this.charts[key]=new Chart(canvas,{
      ...
      data:{labels:months.map(x=>x.label),datasets:[
        {label:'Receitas',data:inc,backgroundColor:'rgba(34,197,94,0.75)',borderRadius:6,borderSkipped:false},
        {label:'Despesas',data:exp,backgroundColor:'rgba(240,77,77,0.75)',borderRadius:6,borderSkipped:false}
      ]},
      options:{responsive:true,plugins:{legend:{labels:{color:'#7c84a0',font:{family:'Inter',size:11}}}},
        scales:{x:{ticks:{color:'#7c84a0',font:{size:11}},grid:{color:'#191c28'}},y:{ticks:{color:'#7c84a0',font:{size:11},callback:v=>'R$'+(v>=1000?(v/1000).toFixed(0)+'k':v)},grid:{color:'#191c28'}}}}
    });
  },
  renderDonut(byCategory) {
    ...
    document.getElementById('donut-legend').innerHTML=labels.map((l,i)=>`<div class="legend-item"><div class="legend-dot" style="background:${colors[i]}"></div><span class="legend-label">${l}</span><span class="legend-val">${Utils.formatCurrency(values[i])}</span><span class="text-muted text-sm">${Math.round(values[i]/total*100)}%</span></div>`).join('');
  },
  renderPie(data) {
    const wrap=document.getElementById('pie-wrap');
    if(!wrap) return;
    if(!data.income&&!data.expense){wrap.innerHTML='<div class="empty-state" style="padding:30px"><div class="empty-icon">🥧</div><div class="empty-title">Sem dados</div></div>';return;}
    wrap.innerHTML='<canvas id="chart-pie"></canvas><div id="pie-legend" class="donut-legend mt-2"></div>';
    const canvas=document.getElementById('chart-pie');
    if(this.charts.pie) this.charts.pie.destroy();
    this.charts.pie=new Chart(canvas,{type:'doughnut',data:{labels:['Receitas','Despesas'],datasets:[{data:[data.income,data.expense],backgroundColor:['rgba(34,197,94,0.8)','rgba(240,77,77,0.8)'],borderWidth:0,hoverOffset:8}]},options:{responsive:true,cutout:'65%',plugins:{legend:{display:false}}}});
